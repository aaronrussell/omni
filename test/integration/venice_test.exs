defmodule Integration.VeniceTest do
  use ExUnit.Case, async: true

  alias Omni.{Context, Message, Provider, Response, StreamingResponse}
  alias Omni.Content.{Text, Thinking, ToolUse}

  @text_fixture "test/support/fixtures/sse/venice_text.sse"
  @tool_use_fixture "test/support/fixtures/sse/venice_tool_use.sse"
  @thinking_fixture "test/support/fixtures/sse/venice_thinking.sse"

  setup_all do
    Provider.load([:venice])
    :ok
  end
  ...
  describe "generate_text/3 — tool use" do
    test "returns a tool use response" do
      stub_fixture(:int_venice_tool, @tool_use_fixture)
  ...
  describe "generate_text/3 — thinking" do
    test "returns thinking and text content" do
      stub_fixture(:int_venice_thinking, @thinking_fixture)

      assert {:ok, %Response{} = resp} =
               Omni.generate_text(model(), "How many R's are in strawberry?",
                 api_key: "test-key",
                 thinking: :high,
                 plug: {Req.Test, :int_venice_thinking}
               )
  ...
  describe "stream_text/3 — text streaming" do
    test "streams text and completes with full response" do
      stub_fixture(:int_venice_stream, @text_fixture)

      {:ok, sr} =
        Omni.stream_text(model(), "Write a haiku about why the sky is blue.",
          api_key: "test-key",
          plug: {Req.Test, :int_venice_stream}
        )
    ...
    test "complete/1 returns a full response" do
      stub_fixture(:int_venice_complete, @text_fixture)

      {:ok, sr} =
        Omni.stream_text(model(), "Write a haiku about why the sky is blue.",
          api_key: "test-key",
          plug: {Req.Test, :int_venice_complete}
        )
