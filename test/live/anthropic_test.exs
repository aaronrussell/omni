defmodule Live.AnthropicTest do
  ...
  test "text generation" do
    ...
    IO.puts(
      "[anthropic] text: \"#{truncated}\" (stop: #{resp.stop_reason}, in: #{resp.turn.usage.input_tokens}, out: #{resp.turn.usage.output_tokens})"
    )
  end
  ...
  test "tool use" do
    ...
    IO.puts(
      "[anthropic] tool: #{tool_use.name}(#{inspect(tool_use.input)}) (stop: #{resp.stop_reason})"
    )
  end
  ...
  test "thinking" do
    ...
    IO.puts("[anthropic] thinking: \"#{truncated}\" (#{String.length(thinking_text)} chars)")
    IO.puts("[anthropic] answer: \"#{String.replace(answer, "\n", " ")}\"")
  end
end
