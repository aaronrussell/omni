...
├── turn.ex                         # Turn struct (messages + usage, used in MessageTree and Response)
...
- `%Response{}` carries a `%Turn{}` struct (via `response.turn`) containing all messages from the generation and their cumulative usage. The `turn.id` and `turn.parent` fields position it within a conversation tree. For stateless calls, defaults are `id: 0, parent: nil`. Token usage is accessed via `response.turn.usage`, not directly on the response.
- `%MessageTree{}` stores turns as `%Turn{}` structs keyed by `Turn.id()`. The field is `tree.turns` (not `rounds`). `push/3` returns `{%Turn{}, updated_tree}`.
- The agent's public API for mutating configuration is `set_state/2` (keyword list, replaces by key, atomic) and `set_state/3` (single field + value or function). Settable fields: `:model`, `:system`, `:tools`, `:tree`, `:opts`, `:meta`. The agent has no `session_id` — session identity is an application concern. Storage/persistence is not built into the agent; events carry enough context (Turn id/parent on `:done` and `:turn` events) for external listeners to persist.
...
- **`context/sessions.md`** — Session persistence design: MessageTree (Layer 1, complete), agent state restructure (Layer 2, complete). Serialization (Layer 3, future).
