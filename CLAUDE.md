...
- **Streaming-first**: Every LLM request uses streaming HTTP via Req's `into: :self` async mode. The event pipeline is composed as a lazy `Stream`: SSE parsing → `Provider.parse_event/2` (adapt + dialect parse) → delta tuples. `parse_event` returns a list of `{type, map}` tuples (4 generic types: `:message`, `:block_start`, `:block_delta`, `:error`), so the pipeline uses `Stream.flat_map/2`. `StreamingResponse.start/2` takes a `%Req.Request{}` and `%Model{}`, executes the request, and builds this pipeline internally. Its `Enumerable` implementation wraps the pipeline, accumulates a partial `%Response{}`, and yields `{event_type, event_map, partial_response}` tuples. Key functions: `to_stream/1` (explicit `Stream` conversion), `text_stream/1` (stream of text delta binaries), `complete/1` (consume to final `%Response{}`), `cancel/1` (cancel async response). `new/1` is a simple struct constructor for testing with scripted delta lists. No spawned process — Req/Finch manage the connection.
