...
- `%MessageTree{}` stores individual messages as nodes with `{id, parent, message}`. The struct field is `tree.nodes`. `push/2` takes a single message and returns `{id, updated_tree}`. A "turn" is a conceptual grouping (user message through final assistant response), not a data type.
