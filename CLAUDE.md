...
- Providers use `use Omni.Provider, dialect: Module` — the macro generates `dialect/0` and defaults for all optional callbacks. Provider IDs are assigned in the application config, not on the module. Provider registration: `config :omni, :providers, [:anthropic, :openai, custom: MyApp.Custom]`. Shorthand atoms are normalized to `{id, Module}` tuples at startup. Models are stored in `:persistent_term` keyed as `{Omni, provider_id}`, written once at startup by `Omni.Application`.
