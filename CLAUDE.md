...
- `%Response{}` carries `messages` (list of messages from the generation) and `usage` (cumulative `%Usage{}`), accessed directly as `response.messages` and `response.usage`.
- `%MessageTree{}` stores `%Message{}` structs directly as nodes, keyed by integer ID in `tree.nodes`. `push/2` stamps the message's `node` field with `%{id: id, parent_id: parent_id}` and returns `{id, updated_tree}`. Messages outside a tree have `node: nil`. The `tree_node` type on MessageTree defines the position metadata shape. Enumerable yields `%Message{}` structs along the active path. A "turn" is a conceptual grouping (user message through final assistant response), not a data type.
- The agent's public API for mutating configuration is `set_state/2` (keyword list, replaces by key, atomic) and `set_state/3` (single field + value or function). Settable fields: `:model`, `:system`, `:tools`, `:tree`, `:opts`, `:meta`. The agent has no `session_id` — session identity is an application concern. Storage/persistence is not built into the agent; events carry enough context (message ids on `:done` and `:turn` events) for external listeners to persist.
